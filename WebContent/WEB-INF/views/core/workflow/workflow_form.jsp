...
<table border="0" cellpadding="0" cellspacing="0" class="in-tb margin-top5">
  ...
  <tr>
    <td class="in-lab" width="15%"><em class="required">*</em><s:message code="workflow.step"/>:</td>
    <td class="in-ctt" width="85%" colspan="3">
    	<div><input type="button" onclick="$('#stepContainer').append($('#stepTemplate').val());" value="<s:message code='addNewLine'/>"/></div>
    	<div id="stepContainer">
    		<c:forEach var="step" items="${bean.steps}">
    		<div>
    			<input type="input" name="stepName" value="<c:out value='${step.name}'/>" style="width:150px;"/> &nbsp;
    			<select name="stepRoleId">
    				<f:options items="${roleList}" selected="${step.role.id}" itemLabel="name" itemValue="id"/>
    			</select> &nbsp;
    			<a href="javascript:void(0);" onclick="$(this).parent().remove();" class="ls-opt"><s:message code="delete"/></a>
    		</div>
    		</c:forEach>
    	</div>
    	<textarea id="stepTemplate" style="display:none;">
	    	<div>
	   			<input type="input" name="stepName" style="width:150px;"/> &nbsp;
	   			<select name="stepRoleId">
