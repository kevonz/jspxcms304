...
$(function() {
	...
	var zNodes =[
  	...
  	{"id":${item.id},"pId":<c:out value="${item.parent.id}" default="-1"/>,"name":"${item.name}","displayName":"${item.displayName}",<c:choose><c:when test="${empty item.parent}">"open":true</c:when><c:otherwise>"open":false</c:otherwise></c:choose>}<c:if test="${!status.last}">,</c:if>
  	</c:if>
  	</c:forEach>
  ];
